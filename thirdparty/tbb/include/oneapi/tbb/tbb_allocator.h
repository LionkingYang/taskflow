...
class tbb_allocator {
 public:
  ...
  //! Always defined for TBB containers (supported since C++17 for std
  //! containers)
  ...
  void deallocate(T* p, std::size_t) { r1::deallocate_memory(p); }
...
inline bool operator==(const tbb_allocator<T>&,
                       const tbb_allocator<U>&) noexcept {
  return true;
}
