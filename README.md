...
1. 修改json文件，注意修改后的json文件应该合法无循环依赖，可以用check_ops.py进行检测。（**对于以上的变动2，这步操作后会报算子func找不到的error，可以忽略**)
2. 发布新的算子so到算子目录。

**如上，涉及图结构的变动，是比较危险的，操作也比较复杂，因此不建议重要业务采取如此的方式，建议在验证完json和算子的合法性之后，关机更新。如果业务必须热更新，可以先准备好更新后的json和op文件，用check_ops.py和check_json_file.py检查无告警后再进行以上的两步热更新操作**
