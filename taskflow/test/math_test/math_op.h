...
BeginFunc(a) int res = Input(int) + 1;
WriteToOutput(a, res, int);
EndFunc;

BeginFunc(b) int res = ReadTaskOutput(a, int) + 1;
WriteToOutput(b, res, int);
EndFunc;

BeginFunc(c) int res = ReadTaskOutput(a, int) + 1;
WriteToOutput(c, res, int);
EndFunc;

BeginFunc(d) int res = ReadTaskOutput(b, int) + ReadTaskOutput(c, int);
WriteToOutput(d, res, int);
EndFunc;

BeginFunc(e) int res = ReadTaskOutput(d, int) + 1;
WriteToOutput(e, res, int);
EndFunc;

BeginFunc(f) int res = ReadTaskOutput(e, int) + 1;
